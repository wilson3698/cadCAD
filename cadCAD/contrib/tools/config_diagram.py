...
def extract_keys(f: callable) -> dict:
    ...
    output = {"state": extract_vars_from_source(src, state_key),
...
def generate_relations(config: Configuration) -> list:
    """
...
def generate_params_cluster(params: dict, i: int) -> Digraph:
    ...
    params_graph.attr(style='filled, dashed', label="Parameters",
                     fillcolor='skyblue')
    for key, value in params.items():
        name = "param_{}_{}".format(key, i)
        description = "{} ({})".format(key, type(value).__name__)
        params_graph.node(name, description,
                         shape='cylinder',
                         style='filled, solid',
                         fillcolor='honeydew'),
    return params_graph
...
def generate_psub_graph(i: int):
    ...
    psub_graph.attr(style='filled, dashed', label="Partial State Update Block",
                    fillcolor='thistle', center='true')
    return psub_graph


def relate_variables_to_state(graph: Digraph, variables: dict, i: int,
                              suffix='', reverse=False, dst_i=None) -> Digraph:
    if dst_i is None:
        dst_i = i
...
def generate_policies_cluster(policies:dict, i: int) -> Digraph:
    policy_graph = Digraph("cluster_policy_{}".format(i))
    policy_graph.attr(label="Policies")
    policy_graph.node("agg_{}".format(i), "Aggregation", shape='circle',
                    style='filled,bold', fillcolor='greenyellow', width="1")
    for key, value in policies.items():
        name = "policy_{}_{}".format(key, i)
        description = "{} ({})".format(key, value.__name__)
        policy_graph.node(name, description, style='filled, bold',
                          fillcolor='palegreen', shape='cds',
                          height="1", width="1")
        policy_graph.edge(name, 'agg_{}'.format(i))
    return policy_graph


def relate(graph, relations, i, src_prefix, dst_prefix, suffix='', reverse=False) -> Digraph:
...
def generate_sufs_cluster(sufs:dict, i: int, agg=False) -> Digraph:
    ...
    for key, value in sufs.items():
        ...
        description = "{} ({})".format(key, value.__name__)
        suf_graph.node(name, description, style='filled, bold',
                          fillcolor='red', shape='cds',
                          height="1", width="1")
        if agg:
            suf_graph.edge("agg_{}".format(i), name)
    return suf_graph
...
def generate_diagram(config: Configuration):
    """
    Generates an diagram for an cadCAD configuration object.
    """
    relations = generate_relations(config)
    initial_state = config.initial_state
    params = config.sim_config['M']
    time_graph = generate_time_graph()
    psubs = config.partial_state_updates
    for i, psub in enumerate(psubs):
        psub_graph = generate_psub_graph(i)
        psub_graph.node('state_{}'.format(i), "State {}".format(i),
                         shape='cylinder',
                         style='filled, solid',
                         fillcolor='honeydew')

        psub_graph.node('state_{}'.format(i), "State {}".format(i + 1),
                         shape='cylinder',
                         style='filled, solid',
                         fillcolor='honeydew')
        ...
        psub_map = relations[i].get('map', {})
        policy_map = psub_map.get('policies', {})
        policy_inputs = {policy: relation['state'] for policy, relation in policy_map.items()}
        policy_params = {policy: relation['params'] for policy, relation in policy_map.items()}
        ...
        if len(list_of_inputs) > 0:
            ...
            for policy in policies.keys():
                psub_graph.edge("state_{}".format(i), "policy_{}_{}".format(policy, i))
        else:
            agg = False
        ...
        sufs_inputs = {policy: relation['state'] for policy, relation in suf_map.items()}
        sufs_params = {policy: relation['params'] for policy, relation in suf_map.items()}
        ...
        if len(list_of_inputs) > 0:
            ...
            for suf in sufs.keys():
                psub_graph.edge("suf_{}_{}".format(suf, i), "state_{}".format(i + 1))
                if agg is False:
                    psub_graph.edge("state_{}".format(i), "suf_{}_{}".format(suf, i))
